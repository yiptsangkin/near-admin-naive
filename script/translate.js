    ...
    void async function () {
        ...
        currentLocaleList.forEach(function (citem) {
            if (withBaseFile(citem)) {
                ...
                let uiName = defaultName.replace('_', '')
                let dbUiName = `${uiName.charAt(0).toUpperCase()}${uiName.slice(1)}`
                uiLocale.push(`${uiName}, date${dbUiName}`)
                customLocale.push(`import custom${dbUiName} from '@locale/${citem}'`)
                customOpt.push(`[custom${dbUiName}.locale]: custom${dbUiName}`)
                uiOpt.push(`[custom${dbUiName}.locale]: { locale: ${uiName}, dateLocale: date${dbUiName} }`)
            }
        })
        const uniLocaleStr = `import { ${uiLocale.join(', ')} } from 'naive-ui'`
        // read lang.tpl
